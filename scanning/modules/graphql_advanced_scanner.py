...
class GraphQLAdvancedScanner(ScanModule):
    ...
    # Common GraphQL endpoints - Extended
    GRAPHQL_ENDPOINTS = [
        "/graphql",
        "/graphql/",
        "/api/graphql",
    ...
    async def scan(
        ...
        async with httpx.AsyncClient(verify=False, timeout=self.timeout) as client:
            ...
            if self.schema_info and self.schema_info.backend_type == "hasura":
            ...
            if self.schema_info and self.schema_info.backend_type == "supabase":
                    ...
                    if "data" in data and len(data["data"]) > 500:
                        ...
                        findings.append(Finding(
                            ...
                            evidence=[
                                ...
                                f"Response fields: {len(data.get('data', {}))}",
                            ],
                            cwe="CWE-400",
                            cvss_score=6.5,
        ...
        for payload in directive_queries:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    # Check if directive was accepted
    ...
    async def _test_circular_fragments(
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # If no error about circular reference
        ...
        for payload in idor_queries:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if "data" in data and data["data"]:
                        # Check if data was returned for arbitrary ID
                        data_str = str(data["data"]).lower()
                        
                        if "email" in data_str or "user" in data_str:
                            findings.append(Finding(
                                name="GraphQL IDOR - User Data Exposure",
                                severity="HIGH",
                                confidence="MEDIUM",
                                description="GraphQL returns user data without proper authorization",
                                matched_at=graphql_url,
                                evidence=[
                                    f"Query: {payload['query'][:50]}...",
                                    "User data returned for arbitrary ID",
                                ],
                                cwe="CWE-639",
                                cvss_score=7.5,
        ...
        for payload in injection_payloads:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    elif "$ne" in str(payload) and "data" in data and data["data"]:
                        findings.append(Finding(
            ...
            for filter_payload in RLS_FILTER_BYPASS_PAYLOADS[:5]:  # Limit payloads
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        # Check if we got data (bypass worked)
                        if "data" in data and data["data"]:
        ...
        for mutation in mutation_tests[:10]:  # Limit tests
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    # Check if mutation was accepted (not just error)
                    if "data" in data and data["data"]:
